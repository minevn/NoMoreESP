...
public class HealthHider implements Listener {
	...
	private static final PacketType[] ENTITY_PACKETS = { SPAWN_ENTITY_LIVING, NAMED_ENTITY_SPAWN, ENTITY_METADATA };
		...
		return new PacketAdapter(plugin, ENTITY_PACKETS) {
			...
			public void onPacketSending(PacketEvent event) {
				...
				if (e instanceof LivingEntity && packet.getWatchableCollectionModifier().read(0) != null
						&& e.getType() != EntityType.HORSE && e.getType() != EntityType.PIG
						&& e.getType() != EntityType.WOLF && e.getUniqueId() != event.getPlayer().getUniqueId()) {
			...
			private void processDataWatcher(WrappedDataWatcher watcher) {
				if (watcher != null && watcher.hasIndex(7) && watcher.getFloat(7) != 0) { // 2
																							// is
																							// name
																							// index
